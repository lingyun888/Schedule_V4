class InitFlowFilter:
    ...
    def init_flows_filter(self):
        ...
        for flow, path in self.flow_paths_dic.items():
            ...
            # 計算path size
            mentain_time_dict[flow] = {"Ingress": path[0]["Ingress"], "Egress": path[0]["Egress"],
                                       "PathSize": len(path)}
        ...
        for i, (flow1, data1) in enumerate(mentain_time_dict.items()):
            for flow2, data2 in list(mentain_time_dict.items())[i + 1:]:
                if data1["Ingress"] == data2["Ingress"] and data1["Egress"] == data2["Egress"] and bool(
                        set(self.flow_paths_dic[flow1][0]['Time']) & set(self.flow_paths_dic[flow2][0]['Time'])):
