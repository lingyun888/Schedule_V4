...
class Demo(QGraphicsView):
    ...
    def draw_grid(self):
        ...
        for i in range(self.grid_size_x + 1):
            line = QGraphicsLineItem(i * self.cell_width, 0, i * self.cell_width,
                                     len(self.grid_size_y) * self.cell_height)
            pen = QPen(self.grid_line_color)
            line.setPen(pen)
            self.scene.addItem(line)
        ...
        for flow_name, path in data_dict.items():
            ...
            for link in path:
                ...
                for time, size_index in x.items():
                    ...
                    if self.is_color_at_position(time * self.cell_width + 0.5,
                                                 y * self.cell_height + 0.5):  # 衝突狀況發生 顯示黑色點點
                        rect = QGraphicsRectItem(time * self.cell_width, y * self.cell_height, 0.3 * self.cell_width,
                                                 0.3 * self.cell_height)
